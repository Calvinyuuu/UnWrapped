  ...
  return (
    <>
      {imgs.map((imgSrc, index) => (
        <div key={items[index].id} className="flex flex-col items-center h-[30vh] w-1/3 rounded-xl bg-neutral-800">
          <motion.div
          ...
          <div className="text-center pt-2 text-wrap">{items[index].name}</div>
          <div className="flex justify-center">
            {items[index].artists.map((artist, index) => (
              <div key={index} className="mx-2 text-wrap">
                {artist.name}
              </div>
            ))}
          </div>
          <div className="text-center text-wrap">{items[index].album.name}</div>
        </div>
      ))}
    </>
  );
